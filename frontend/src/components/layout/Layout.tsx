...
export default function Layout() {
    return (
        <div className="relative flex flex-col min-h-screen w-full max-w-md mx-auto bg-background-light dark:bg-background-dark shadow-2xl overflow-hidden">
            ...
            {/* Bottom Navigation */}
            <nav className="fixed bottom-0 w-full max-w-md bg-surface-dark border-t border-white/5 backdrop-blur-lg z-50">
                <div className="flex items-center justify-around h-[80px] pb-4">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            to={item.to}
                            className={({ isActive }) =>
                                `flex flex-col items-center justify-center gap-1 w-16 h-full transition-colors ${isActive
                                    ? 'text-primary'
